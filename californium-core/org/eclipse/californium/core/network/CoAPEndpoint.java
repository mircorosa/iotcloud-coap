...
public class CoAPEndpoint implements Endpoint {
	...
	public synchronized void start() throws IOException {
		if (started) {
			//TODO Logger.log(Level.FINE, "Endpoint bound to " + getAddress().toString() + " is already started");
			return;
		}
		...
		try {
			//TODO Logger.log(Level.INFO, "Starting Endpoint bound to " + getAddress());
			started = true;
			matcher.start();
			connector.start();
			...
			//TODO Logger.log(Level.SEVERE, "Cannot start Endpoint at " + getAddress(), e);
			stop();
			throw e;
		}
	}
	...
	public synchronized void stop() {
		if (!started) {
			//TODO Logger.log(Level.INFO, "Endpoint at address " + getAddress() + " is already stopped");
		} else {
			//TODO Logger.log(Level.INFO, "Stopping endpoint at address " + getAddress());
			started = false;
			connector.stop();
			matcher.stop();
			for (EndpointObserver obs:observers)
	...
	public synchronized void destroy() {
		//TODO Logger.log(Level.INFO, "Destroying endpoint at address " + getAddress());
		if (started)
			stop();
		connector.destroy();
		for (EndpointObserver obs:observers)
			...
			if (parser.isRequest()) {
				...
				if (!message.isCanceled()) {
					...
					if (message.getType() == Type.CON || message.getType() == Type.NON) {
						...
						//TODO Logger.info("Responding to ping by " + raw.getInetSocketAddress());
